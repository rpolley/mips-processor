...
hex_vals = [i.__hex__()[2:] for i in memory]
...
write_instr_hex = [i.__hex__()[2:] for i in write_instr_vals] #encode values as hex numbers
write_instr = " ".join(write_instr_hex)
#generate the register file harness from the template
template = open("regtest-template.circ", 'r')
gen_file = open("regtest-harness.circ",'w')
for line in template:
	gen_file.write(line.replace("$write_vals",write_vals) #write the values to write to the ram
		.replace("$write_instr",write_instr)) #write instructions to 
